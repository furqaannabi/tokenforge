...
Sample PDFs for exercising the pipeline, covering the paths that matter rather
than variations on the same happy case.
...
| `6-northbridge-past-due-facility.pdf` | **Automatic repayment.** $60,000 at 12.00% per annum over six quarterly instalments, amortising from $11,800 down to $10,300 as the balance falls, lent by CROSSMIND to NORTHBRIDGE — the registered issuer and the registered borrower wallet, so all three provenance verdicts pass. Its first two instalments are already past their due date, which is the whole point: everything else on file matures years out, so `collectible()` is false and a keeper sweep has nothing to do. Mint this one, have the borrower accept and arm the allowance, and a collection can happen immediately |
