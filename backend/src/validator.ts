 ...
 *
 * NOTE: `web/lib/validator.ts` currently carries a copy of this logic. The two
 * must not drift; the intended fix is for the web app to import from here once
 * the two packages share a workspace.
 */
...
/** Fields the model was unsure about that a human has not yet confirmed. */
export function fieldsNeedingReview(
  ...
  return (Object.keys(terms) as TermField[]).filter((key) => {
    ...
    return extracted.confidence < LOW_CONFIDENCE_THRESHOLD && !confirmed.has(key);
  });
}
